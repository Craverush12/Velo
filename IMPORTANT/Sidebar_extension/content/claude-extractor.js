/**
 * Claude Conversation Extractor - DOM-resilient extraction for Claude (MAIN world).
 */
(function() {
  'use strict';

  // Prevent double initialization
  if (window.VelocityClaudeExtractor) {
    return;
  }
  ...
  var DataCache = {
     ...
     * Claude uses /chat/{chatId} format
     */
    getChatIdFromUrl: function(url) {
      url = url || window.location.href;
      // Claude URL pattern: https://claude.ai/chat/{chatId}
      var match = url.match(/\/chat\/([a-f0-9-]+)/i);
    ...
    checkIsStreaming: function() {
      // Look for streaming indicators (Claude-specific)
      ...
      // Check for stop button visibility (Claude-specific)
   ...
   */
  var STORAGE_KEY = 'velocity_Claude_session';
  ...
  var SessionStore = {
    ...
    init: function() {
      ...
      if (existingData && existingData.sessionId) {
        ...
        if (missingMessages.length > 0) {
          ...
          Logger.warn('      VelocityClaudeExtractor.session.fixMissingMessages()');
    ...
    /**
     * Extract user info, auth token, and trial info from Claude
     */
    extractUserInfo: function() {
      ...
      try {
        ...
        try {
          ...
          for (var i = 0; i < localStorageKeys.length; i++) {
            ...
            if (key.indexOf('user') !== -1 || key.indexOf('auth') !== -1 || key.indexOf('token') !== -1 || 
                key.indexOf('openai') !== -1 || key.indexOf('Claude') !== -1) {
        ...
        try {
          var specificUserKeys = ['userId', 'user_id', 'claude_user_id', 'claude_userId'];
            ...
            if (storedUserId && storedUserId.trim() && !userInfo.user_id) {
              ...
              if (parsedUserId && typeof parsedUserId === 'object') {
                ...
                if (typeof uid === 'string' && uid.trim()) {
                  ...
                  Logger.log('[Claude Extractor] ✅ User identified via localStorage key "' + specificUserKeys[suk] + '": ' + uid.trim());
                  break;
                }
              } else if (typeof storedUserId === 'string' && storedUserId.trim()) {
                userInfo.userId = storedUserId.trim();
                userInfo.user_id = storedUserId.trim();
                Logger.log('[Claude Extractor] ✅ User identified via localStorage key "' + specificUserKeys[suk] + '": ' + storedUserId.trim());
                break;
              }
            }
          }
        ...
        // Method 3: Check window object (Claude might expose user data)
        try {
        ...
        // Method 6: Check for trial/subscription/usage info in Claude's data
        try {
    ...
    exportSessionAsync: function(callback) {
      ...
      // Get ONLY Chrome storage data (Velocity extension) - skip Claude page extraction
  ...
  function findConversationRoot() {
    ...
    // Strategy 3: Look for Claude's specific message container structure
    // Claude often uses a scrollable container with message groups
    ...
    // Strategy 5: Try Claude-specific selectors (more specific first)
  ...
  function collectMessageCandidates(root) {
    ...
    // Strategy 1b: Claude-specific message containers (expanded)
    var claudeMessages = root.querySelectorAll(
    ...
    Logger.log('Strategy 1b (Claude message containers): ' + claudeMessages.length + ' containers, added ' + (containersAfter - containersBefore) + ' candidates');
    ...
    // Strategy 7: Claude-specific - Look for divs with substantial text content (aggressive fallback)
    if (candidates.length === 0) {
      Logger.log('Strategy 7 (Claude aggressive): Searching for divs with text content...');
      ...
      Logger.log('Strategy 7 (Claude aggressive): found ' + textDivs.length + ' text divs, added ' + addedCount + ' candidates');
    }
  ...
  function isJavaScriptNoise(text) {
    ...
    var jsPatterns = [
      'window.__oai_',           // Claude analytics
      'window.__oai_log',        // Claude logging
  ...
  /**
   * Extracts artifacts/canvas from the page (Claude specific)
   * @param {HTMLElement} element - Message element
  ...
  function filterRealMessages(candidates) {
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      if (!text || text.trim().length === 0) { 
        // For Claude: Check if children have text (containers might not have direct text)
        var childText = '';
  ...
  function detectMessageRole(element, index) {
    ...
    var hasAssistantIndicators = element.querySelector('[data-testid*="assistant"]') ||
                                  ...
                                  element.querySelector('img[alt*="Claude"]') ||
  ...
  // Per-page throttle (same pattern as chatgpt-extractor.js). Prevents the
  // host page from lagging when Claude's router fires many pushState events.
  ...
  function extractConversation() {
    ...
    Logger.info('═══════════════════════════════════════════════════════════');
    Logger.info('Starting Claude Conversation Extraction');
    Logger.info('═══════════════════════════════════════════════════════════');
    ...
    try {
      ...
      if (candidates.length === 0) {
        Logger.warn('No message candidates found - trying aggressive Claude fallback...');
        
        // Aggressive Claude fallback: Look for any div with substantial text
        ...
        if (fallbackCandidates.length > 0) {
          Logger.log('Claude fallback found ' + fallbackCandidates.length + ' text containers');
      ...
      if (realMessages.length === 0) {
        ...
        // Strategy B: Look for elements with message-like structure (Claude-specific)
  ...
  function extractSidebar() {
    ...
    try {
      // Claude sidebar strategies - try multiple approaches
      ...
      if (nav) {
        ...
        // Claude uses /chat/[id] pattern
        chatLinks = nav.querySelectorAll('a[href*="/chat/"]');
        Logger.log('Strategy 1: Found nav with ' + chatLinks.length + ' chat links');
      }
      ...
      if (chatLinks.length === 0) {
        ...
        for (var i = 0; i < sidebarSelectors.length; i++) {
          ...
          if (sidebar) {
            chatLinks = sidebar.querySelectorAll('a[href*="/chat/"]');
            if (chatLinks.length > 0) {
      ...
      // Strategy 3: Look for any element with chat links (fallback)
      if (chatLinks.length === 0) {
        chatLinks = document.querySelectorAll('a[href*="/chat/"]');
        if (chatLinks.length > 0) {
          Logger.log('Strategy 3: Found ' + chatLinks.length + ' chat links in document');
        }
      }
      ...
      chatLinks.forEach(function(link, index) {
        ...
        // Extract chat ID from href (Claude uses /chat/[id] pattern)
        // e.g., /chat/0697c068-48d3-4c25-b4fa-5f4fd24286d0
        var chatIdMatch = href.match(/\/chat\/([a-f0-9-]+)/i);
        ...
        // Build full URL
        var fullUrl = href.startsWith('http') ? href : 'https://claude.ai' + href;
        
        // Check if this is the current active chat
        var currentPath = window.location.pathname;
        var isActive = currentPath === href || currentPath.indexOf('/chat/' + chatId) !== -1;
  ...
  function extractAll() {
    Logger.info('═══════════════════════════════════════════════════════════');
    Logger.info('Extracting Full Claude Data (Conversation + Sidebar)');
    Logger.info('═══════════════════════════════════════════════════════════');
  ...
  if (typeof window.SmartTriggerManager !== 'undefined') {
    ...
    if (window.VelocityClaudeExtractor) {
      window.VelocityClaudeExtractor.smartTrigger = smartTrigger;
    }
  } else {
  ...
  var SmartTracker = {
    ...
    start: function(callback) {
      ...
      // Only run when actually on a chat conversation page (e.g. /chat/<id>) —
      // avoids hammering the host main thread on home/new-chat URLs.
      if (DataCache.isConversationPage()) {
  ...
  window.VelocityClaudeExtractor = {
     ...
     * Can be called from browser console: VelocityClaudeExtractor.triggerContextEngine()
     */
    triggerContextEngine: function() {
      log('[Claude Extractor] 🚀 Manual ContextEngine API trigger initiated');
      ...
      try {
        ...
        if (!conversationData.success || !conversationData.messages || conversationData.messages.length === 0) {
          error('[Claude Extractor] ❌ No conversation data to process');
          return { success: false, error: 'No conversation data available' };
        }

        log('[Claude Extractor] 📊 Extracted data for ContextEngine:', {
        ...
        function handleBridgeResponse(event) {
          if (event.data.type === 'CONTEXT_ENGINE_BRIDGE_RESPONSE' &&
            ...
            if (event.data.error) {
              error('[Claude Extractor] ❌ Bridge error:', event.data.error);
            } else if (event.data.response && event.data.response.success) {
              log('[Claude Extractor] ✅ ContextEngine API triggered successfully');
              log('[Claude Extractor] 📄 API Response:', event.data.response.contextResult);
            } else {
              error('[Claude Extractor] ❌ ContextEngine API failed:', event.data.response?.error);
            }
          }
        }
        ...
        setTimeout(function() {
          window.removeEventListener('message', handleBridgeResponse);
          warn('[Claude Extractor] ⏰ Bridge response timeout for manual trigger');
        }, 10000);

        return { success: true, message: 'ContextEngine API trigger initiated' };

      } catch (error) {
        error('[Claude Extractor] 💥 Error triggering ContextEngine API:', error);
    ...
    showJSON: function(data, label) {
      if (!data) {
        warn('[Claude Extractor] No data provided');
        return;
      }
      // Validate data structure before showing
      if (data.messages && !Array.isArray(data.messages)) {
        warn('[Claude Extractor] ⚠️ Warning: messages is not an array');
        data.messages = [];
      }
  ...
  Logger.success('Claude Extractor initialized');
  ...
  function autoStartTracking() {
    if (!CONFIG.AUTO_TRACK) {
      Logger.info('Auto-tracking disabled. Use VelocityClaudeExtractor.startTracking() manually.');
      return;
    }
    ...
    Logger.info('');
    Logger.info('📦 View collected: VelocityClaudeExtractor.session.summary()');
    Logger.info('💾 Export all: VelocityClaudeExtractor.session.export()');
    Logger.info('═══════════════════════════════════════════════════════════');
  }
  ...
  if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
      if (request.type === 'CLEANUP_EXTENSION_DATA') {
        log('[Claude Extractor] 🧹 Received cleanup message, resetting session data');
        ...
        try {
          ...
          var STORAGE_KEY = 'VelocityClaudeExtractor_session';
          localStorage.removeItem(STORAGE_KEY);

          log('[Claude Extractor] ✅ Session data cleared');
        } catch (e) {
          error('[Claude Extractor] ❌ Error clearing session data:', e);
        }
