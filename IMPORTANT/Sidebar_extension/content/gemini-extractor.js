/**
 * Gemini Conversation Extractor - DOM-resilient extraction for Gemini (MAIN world).
 */
(function() {
  'use strict';

  // Prevent double initialization
  if (window.VelocityGeminiExtractor) {
    return;
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // CONSOLE HELPERS
  // Quiet by default in Sidebar_extension. Set localStorage.velocity_sidebar_debug = '1'
  // (in the host page DevTools) to re-enable verbose logging.
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  var Compression = {
    ...
    _lzDecompress: function(compressed) {
      var data = JSON.parse(atob(compressed));
      var dict = {};
  ...
  var DataCache = {
     ...
     * Gemini uses /app/{chatId} format
     */
    getChatIdFromUrl: function(url) {
      url = url || window.location.href;
      // Gemini URL pattern: https://gemini.google.com/app/{chatId}
      var match = url.match(/\/app\/([a-f0-9]+)/i);
    ...
    checkIsStreaming: function() {
      // Look for streaming indicators (Gemini-specific)
      ...
      // Check for stop button visibility (Gemini-specific)
      var stopBtn = document.querySelector('button[data-testid="stop-button"], button[aria-label*="Stop"], button[aria-label*="stop"]');
     ...
     */
    // Re-entry guard: see chatgpt-extractor.js for rationale. Prevents parallel
    // 200ms-poll loops from spawning when DOM mutations are flooding in.
    ...
    waitForStreamingComplete: function(callback, maxWait) {
      ...
      function check() {
        if (!self.checkIsStreaming()) {
          if (CONFIG.VERBOSE) { Logger.success('Streaming completed'); }
          setTimeout(flushCallbacks, 100);
          return;
        }
        if (Date.now() - startTime > maxWait) {
          if (CONFIG.VERBOSE) { Logger.warn('Streaming timeout, extracting partial'); }
          flushCallbacks();
          return;
        }
    ...
    setConversationCache: function(chatId, data) {
      ...
      Logger.log('Cache updated for chat: ' + chatId + ' (hash: ' + hash + ')');
    },
    ...
    hasConversationChanged: function(chatId, newData) {
      ...
      if (changed) {
        Logger.log('Conversation changed: old=' + cached.hash + ', new=' + newHash);
      }
    ...
    setSidebarCache: function(data) {
      ...
      Logger.log('Sidebar cache updated (hash: ' + hash + ', chats: ' + this.sidebar.chatCount + ')');
    },
    ...
    clearChat: function(chatId) {
      if (this.conversations[chatId]) {
        delete this.conversations[chatId];
        Logger.info('Cache cleared for chat: ' + chatId);
      }
    },
   ...
   */
  var STORAGE_KEY = 'velocity_gemini_session';
  ...
  var SessionStore = {
    ...
    init: function() {
      ...
      if (existingData && existingData.sessionId) {
        ...
        Logger.success('📂 Resumed existing session: ' + this.sessionId);
        Logger.log('   Loaded ' + this.visitOrder.length + ' chats, ' + this.stats.totalMessagesCollected + ' messages');
        ...
        if (missingMessages.length > 0) {
          ...
          Logger.warn('      VelocityGeminiExtractor.session.fixMissingMessages()');
    ...
    saveToStorage: function() {
      ...
      setTimeout(function() {
        try {
          ...
          // Extract user information to include in saved session data
    ...
    upsertConversation: function(chatId, data) {
      if (!chatId || !data) return { action: 'skipped', reason: 'invalid data' };
      ...
      if (!existing) {
        ...
        Logger.success('📥 NEW chat added to session: ' + this.conversations[chatId].title);
    ...
    /**
     * Extract user info, auth token, and trial info from Gemini
     */
    extractUserInfo: function() {
      ...
      try {
        ...
        try {
          ...
          for (var i = 0; i < localStorageKeys.length; i++) {
            ...
            if (key.indexOf('user') !== -1 || key.indexOf('auth') !== -1 || key.indexOf('token') !== -1 || 
                key.indexOf('openai') !== -1 || key.indexOf('Gemini') !== -1) {
        ...
        try {
          var specificUserKeys = ['userId', 'user_id', 'gemini_user_id', 'gemini_userId'];
            ...
            if (storedUserId && storedUserId.trim() && !userInfo.user_id) {
              ...
              if (parsedUserId && typeof parsedUserId === 'object') {
                ...
                if (typeof uid === 'string' && uid.trim()) {
                  ...
                  Logger.log('[Gemini Extractor] ✅ User identified via localStorage key "' + specificUserKeys[suk] + '": ' + uid.trim());
                  break;
                }
              } else if (typeof storedUserId === 'string' && storedUserId.trim()) {
                userInfo.userId = storedUserId.trim();
                userInfo.user_id = storedUserId.trim();
                Logger.log('[Gemini Extractor] ✅ User identified via localStorage key "' + specificUserKeys[suk] + '": ' + storedUserId.trim());
                break;
              }
            }
          }
        ...
        // Method 3: Check window object (Gemini might expose user data)
        try {
        ...
        // Method 6: Check for trial/subscription/usage info in Gemini's data
        try {
    ...
    exportSessionAsync: function(callback) {
      ...
      // Get ONLY Chrome storage data (Velocity extension) - skip Gemini page extraction
    ...
    fixMissingMessages: function() {
      ...
      if (needsNavigation.length > 0) {
        ...
        needsNavigation.forEach(function(item, index) {
          ...
          log('      URL: ' + item.url);
          log('      → Click to navigate, then run: VelocityDemo.fixMissing()');
        });
      }
    ...
    listMissingMessages: function() {
      ...
      if (missing.length > 0) {
        ...
        missing.forEach(function(item, index) {
          log('   ' + (index + 1) + '. "' + item.title + '"');
          log('      Expected: ' + item.messageCount + ' messages');
          log('      URL: ' + item.url);
        });
    ...
    debug: function() {
      log('\n═══════════════════════════════════════════════════════════════');
      log('🔍 SESSION STORE DEBUG');
      log('═══════════════════════════════════════════════════════════════');
      log('Session ID:', this.sessionId);
      log('Visit Order:', this.visitOrder);
      log('Conversations in memory:', Object.keys(this.conversations).length);
      ...
      Object.keys(this.conversations).forEach(function(chatId) {
        var chat = self.conversations[chatId];
        log('  • ' + chatId.substring(0, 8) + '...: "' + chat.title + '" (' + chat.messageCount + ' msgs)');
      });
      console.log('');
      ...
      try {
        ...
        if (stored) {
          ...
          log('  Conversations stored:', Object.keys(parsed.conversations || {}).length);
          log('  Visit order:', parsed.visitOrder);
        } else {
          log('localStorage: EMPTY');
        }
      } catch (e) {
  ...
  var Logger = {
    prefix: '[Gemini Extractor]',
    
    // Verbose logs - only shown when VERBOSE is true
    log: function() {
      if (CONFIG.VERBOSE) {
    ...
    // Info - only shown when VERBOSE is true
    info: function() {
      if (CONFIG.VERBOSE) {
        var args = Array.prototype.slice.call(arguments);
    ...
    // Warnings - gated in quiet mode to avoid hammering the host page console.
    warn: function() {
      if (__velocityHostQuiet()) return;
      var args = Array.prototype.slice.call(arguments);
    ...
    // Success - only shown for important events (JSON output, key milestones)
    success: function() {
      var args = Array.prototype.slice.call(arguments);
      var message = args[0] || '';
      // Show if it's about JSON, extraction results, or session data
      if (CONFIG.VERBOSE || 
    ...
    // Debug - only shown when VERBOSE is true
    debug: function() {
      if (CONFIG.VERBOSE) {
        var args = Array.prototype.slice.call(arguments);
        args.unshift(this.prefix + ' 🔍');
    ...
    // JSON output - always shown (important for user)
    ...
    group: function(label) {
      if (CONFIG.VERBOSE) console.group(this.prefix + ' ' + label);
    },
    groupEnd: function() {
      if (CONFIG.VERBOSE) console.groupEnd();
    },
    table: function(data) {
      if (CONFIG.VERBOSE) console.table(data);
    }
  };
  ...
  function findConversationRoot() {
    ...
    if (mainElement) {
      // Check if main contains message-like elements
      var hasMessages = mainElement.querySelector('[class*="Message"], [class*="message"], [data-message-id]');
      if (hasMessages) {
        Logger.success('Found <main> element with messages as root');
        Logger.groupEnd();
        return mainElement;
      }
    }
    ...
    if (roleMain) {
      var hasMessages = roleMain.querySelector('[class*="Message"], [class*="message"], [data-message-id]');
      if (hasMessages) {
        Logger.success('Found [role="main"] element with messages as root');
        Logger.groupEnd();
        return roleMain;
      }
    }
    
    // Strategy 3: Look for Gemini's specific message container structure
    // Gemini often uses a scrollable container with message groups
    var scrollContainers = document.querySelectorAll('[style*="overflow"], [class*="scroll"], [class*="Scroll"]');
    for (var s = 0; s < scrollContainers.length; s++) {
      var scrollContainer = scrollContainers[s];
      // Check if this scroll container has multiple message-like children
      var messageChildren = scrollContainer.querySelectorAll('[class*="MessageGroup"], [class*="message-group"], [data-message-id]');
      if (messageChildren.length >= 2) {
        // This looks like the conversation container
        Logger.success('Found scroll container with ' + messageChildren.length + ' messages');
        Logger.groupEnd();
        return scrollContainer;
      }
    }
    
    // Strategy 4a: Look for parent of elements with data-message-id (most reliable)
    var messageIdElements = document.querySelectorAll('[data-message-id]');
    if (messageIdElements.length >= 1) {
      // Find common parent of all message-id elements
      var firstElement = messageIdElements[0];
      var commonParent = firstElement.parentElement;
      var depth = 0;
      var maxDepth = 15; // Increased depth
      var bestParent = null;
      var bestScore = 0;
      
      while (commonParent && depth < maxDepth && commonParent !== document.body) {
        // Check if this parent contains message-id elements
        var messagesInParent = commonParent.querySelectorAll('[data-message-id]');
        var rect = commonParent.getBoundingClientRect();
        
        // Calculate score: message count + size bonus
        var messageScore = messagesInParent.length;
        var sizeScore = (rect.height > 100 ? 1 : 0) + (rect.width > 200 ? 1 : 0);
        var totalScore = messageScore * 10 + sizeScore;
        
        // Accept if it has messages and reasonable size (more lenient)
        if (messagesInParent.length >= 1 && rect.height > 100 && rect.width > 200) {
      ...
      if (bestParent) {
        var finalMessages = bestParent.querySelectorAll('[data-message-id]');
        Logger.success('Found parent container with ' + finalMessages.length + ' messages (via data-message-id, depth: ' + depth + ')');
        Logger.groupEnd();
        return bestParent;
      }
    }
    
    // Strategy 4b: Look for parent of multiple message groups
    var messageGroups = document.querySelectorAll('[class*="MessageGroup"], [class*="message-group"]');
    if (messageGroups.length >= 1) {
      // Find common parent
      var firstParent = messageGroups[0].parentElement;
      var commonParent = firstParent;
      var depth = 0;
      var maxDepth = 15; // Increased depth
      var bestParent = null;
      var bestScore = 0;
      ...
      while (commonParent && depth < maxDepth && commonParent !== document.body) {
        var groupsInParent = commonParent.querySelectorAll('[class*="MessageGroup"], [class*="message-group"]');
        var rect = commonParent.getBoundingClientRect();
        
        // Calculate score
        var groupScore = groupsInParent.length;
        var sizeScore = (rect.height > 100 ? 1 : 0) + (rect.width > 200 ? 1 : 0);
        var totalScore = groupScore * 10 + sizeScore;
        
        // More lenient: accept if has groups and reasonable size
        if (groupsInParent.length >= 1 && rect.height > 100 && rect.width > 200) {
          if (totalScore > bestScore) {
            bestScore = totalScore;
            bestParent = commonParent;
          }
      ...
      if (bestParent) {
        var finalGroups = bestParent.querySelectorAll('[class*="MessageGroup"], [class*="message-group"]');
        Logger.success('Found parent container with ' + finalGroups.length + ' message groups (depth: ' + depth + ')');
        Logger.groupEnd();
        return bestParent;
      }
    }
    
    // Strategy 5: Try Gemini-specific selectors (more specific first)
    ...
    for (var i = 0; i < conversationSelectors.length; i++) {
      try {
        var elements = document.querySelectorAll(conversationSelectors[i]);
        // Find the one that contains multiple message-like children
        for (var j = 0; j < elements.length; j++) {
          var element = elements[j];
          var rect = element.getBoundingClientRect();
          var messageCount = element.querySelectorAll('[class*="Message"], [data-message-id]').length;
          // More strict: must have multiple messages AND reasonable size
          if (element && rect.height > 200 && rect.width > 300 && messageCount >= 2) {
            Logger.success('Found conversation container via: ' + conversationSelectors[i] + ' (' + messageCount + ' messages)');
            Logger.groupEnd();
            return element;
          }
        }
    ...
    // Strategy 6: Last resort - find any container with message elements (more lenient)
    var allMessageElements = document.querySelectorAll('[data-message-id], [class*="MessageGroup"], [class*="message-group"]');
    if (allMessageElements.length >= 1) {
      // Try to find a container that wraps message elements
      var allContainers = document.querySelectorAll('div, section, article, main');
      var bestContainer = null;
      var bestMsgCount = 0;
      
      for (var k = 0; k < Math.min(allContainers.length, 100); k++) { // Limit search to first 100
        var container = allContainers[k];
        var rect = container.getBoundingClientRect();
        // More lenient size requirements
        if (rect.height > 150 && rect.width > 250) {
          var msgCount = container.querySelectorAll('[data-message-id], [class*="MessageGroup"], [class*="message-group"]').length;
          // Find container with most messages
          if (msgCount > bestMsgCount && msgCount >= 1) {
      ...
      if (bestContainer) {
        Logger.success('Found container with ' + bestMsgCount + ' message elements (fallback strategy)');
        Logger.groupEnd();
        return bestContainer;
      }
    }
    
    Logger.warn('Using document.body as fallback root (may collect too many elements)');
    Logger.groupEnd();
  ...
  function collectMessageCandidates(root) {
    ...
    // Strategy 1b: Gemini-specific message containers (expanded)
    var geminiMessages = root.querySelectorAll(
      '[class*="MessageGroup"], [class*="Message"], [class*="message-group"], ' +
      '[class*="message-content"], [class*="MessageContent"], ' +
      '[class*="block"], [class*="Block"], ' +
      '[class*="content-block"], [class*="ContentBlock"], ' +
      '[class*="gemini-message"], [class*="user-message"], ' +
      '[class*="assistant-message"], [class*="human-message"]'
    );
    var containersBefore = candidates.length;
    geminiMessages.forEach(function(container) {
      // Check if container has substantial text content
      var containerText = container.innerText || container.textContent || '';
      var containerTextTrimmed = containerText.trim();
      
      // Only add container if it has meaningful text (at least 30 chars)
      // This prevents adding empty containers and reduces duplicates
      if (containerTextTrimmed.length >= 30) {
        addCandidate(container);
      } else {
        // Container has no/little text - look for text-containing direct children only
        // Only add direct children (not deeply nested) to avoid duplicates
        var directChildren = Array.from(container.children);
        directChildren.forEach(function(child) {
          if (child.tagName === 'DIV' || child.tagName === 'P') {
            var childText = child.innerText || child.textContent || '';
            var childTextTrimmed = childText.trim();
            if (childTextTrimmed.length >= 20) {
              // Simple noise check
              var tagName = child.tagName.toLowerCase();
              var isNoiseTag = ['nav', 'header', 'footer', 'aside', 'form', 'input', 'textarea', 'button'].indexOf(tagName) !== -1;
              if (!isNoiseTag) {
                addCandidate(child);
              }
            }
    ...
    var containersAfter = candidates.length;
    Logger.log('Strategy 1b (Gemini message containers): ' + geminiMessages.length + ' containers, added ' + (containersAfter - containersBefore) + ' candidates');
    
    // Strategy 2: Elements with message-related data attributes
    var messageDataAttrs = root.querySelectorAll('[data-message-id], [data-testid*="message"], [data-testid*="Message"]');
    ...
    Logger.log('Strategy 3 (article): ' + articles.length + ' elements');
    
    // Strategy 4: Turn-based containers
    var turnContainers = root.querySelectorAll('[data-testid*="turn"], [class*="turn"], [class*="Turn"]');
    ...
    var roleContainers = root.querySelectorAll('[data-role], [role="article"], [role="row"]');
    ...
    var scrollContainers = root.querySelectorAll('[style*="overflow"], [class*="scroll"], [class*="Scroll"]');
    ...
    // Strategy 7: Gemini-specific - Look for divs with substantial text content (aggressive fallback)
    if (candidates.length === 0) {
      Logger.log('Strategy 7 (Gemini aggressive): Searching for divs with text content...');
      var allDivs = root.querySelectorAll('div');
      var textDivs = [];
      var seenText = new Set();
      
      allDivs.forEach(function(div) {
        if (isNoiseElement(div)) return;
        if (!isElementVisible(div)) return;
        
        // Use innerText directly (simpler, doesn't require function hoisting)
        var text = div.innerText || div.textContent || '';
        var trimmedText = text.trim();
        
        // Look for divs with substantial text (at least 20 chars)
        if (trimmedText.length >= 20) {
          // Create a simple hash of the text to avoid duplicates
          var textKey = trimmedText.substring(0, 50);
          if (seenText.has(textKey)) return;
          
          // Check if it's not a child of another candidate
          var isChild = false;
          for (var i = 0; i < candidates.length; i++) {
            if (candidates[i].contains(div)) {
              isChild = true;
              break;
            }
          }
      ...
      // Only add top-level divs (not nested ones)
      var addedCount = 0;
      textDivs.forEach(function(div) {
        var isNested = false;
        for (var j = 0; j < textDivs.length; j++) {
          if (textDivs[j] !== div && textDivs[j].contains(div)) {
            isNested = true;
            break;
          }
        }
      ...
      Logger.log('Strategy 7 (Gemini aggressive): found ' + textDivs.length + ' text divs, added ' + addedCount + ' candidates');
    }
  ...
  function isNoiseElement(element) {
    ...
    var noisePatterns = [
      ...
      'drawer', 'navigation', 'chat-list', 'conversation-list', 'gemini-sidebar',
  ...
  function isJavaScriptNoise(text) {
    ...
    // Check for sidebar/navigation noise (Gemini-specific)
    var sidebarPatterns = [
      'new chat',
      'my stuff',
      'gems',
      ...
      'a* algorithm code implementation'
    ];
    
    // If text contains multiple sidebar patterns, it's likely sidebar content
    var sidebarMatchCount = 0;
    for (var s = 0; s < sidebarPatterns.length; s++) {
      if (lowerText.indexOf(sidebarPatterns[s]) !== -1) {
        sidebarMatchCount++;
      }
    }
    
    // If 2+ sidebar patterns found, it's likely sidebar noise
    if (sidebarMatchCount >= 2) {
      return true;
    }
    
    // Patterns that indicate JavaScript code/analytics
    var jsPatterns = [
      'window.__oai_',           // Gemini analytics
      'window.__oai_log',        // Gemini logging
  ...
  /**
   * Extracts artifacts/canvas from the page (Gemini specific)
   * @param {HTMLElement} element - Message element
  ...
  function filterRealMessages(candidates) {
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      if (!text || text.trim().length === 0) { 
        // For Gemini: Check if children have text (containers might not have direct text)
        var childText = '';
        var textChildren = candidate.querySelectorAll('p, div, span');
        for (var j = 0; j < Math.min(textChildren.length, 10); j++) {
          var child = textChildren[j];
          var childTextContent = child.innerText || child.textContent || '';
          if (childTextContent.trim().length > 0) {
            childText += childTextContent.trim() + ' ';
            if (childText.trim().length >= 10) break; // Found enough text
          }
        }
        
        if (childText.trim().length > 0) {
          text = childText.trim();
          Logger.log('   Found text in children for container: ' + text.substring(0, 50) + '...');
        } else {
          reasons.noText++; 
          reason = 'no-text';
      ...
      // Check for JavaScript code/analytics noise (includes sidebar patterns)
      if (isJavaScriptNoise(text)) {
        reasons.jsNoise++;
        reason = 'js-noise';
        Logger.log('   Filtered JS/sidebar noise: ' + text.substring(0, 50) + '...');
        continue;
      }
      
      // Additional Gemini-specific sidebar content check
      var lowerText = text.toLowerCase();
      var sidebarKeywords = ['new chat', 'my stuff', 'gems', 'chats', 'settings and help', 'upgrade to google ai plus'];
      var sidebarKeywordCount = 0;
      for (var sk = 0; sk < sidebarKeywords.length; sk++) {
        if (lowerText.indexOf(sidebarKeywords[sk]) !== -1) {
          sidebarKeywordCount++;
        }
      }
      // If message contains 2+ sidebar keywords, it's likely sidebar content
      if (sidebarKeywordCount >= 2) {
        reasons.jsNoise++;
        reason = 'sidebar-content';
        Logger.log('   Filtered sidebar content: ' + text.substring(0, 50) + '...');
        continue;
      }
      ...
      // Improved duplicate detection: check prefix and fragment overlap
      ...
      // Check for similar/overlapping messages (fragments)
      // Only mark as fragment if there's significant overlap (80%+ match)
      var isFragment = false;
      var textLength = text.length;
      seenTexts.forEach(function(existingSig) {
        var existingLength = existingSig.length;
        // If this text is a significant portion of existing text (80%+), it's a fragment
        if (existingLength > textLength && textLength >= 20) {
          var overlapRatio = textLength / existingLength;
          if (overlapRatio >= 0.8 && existingSig.indexOf(textSignature.substring(0, Math.min(textLength, 100))) !== -1) {
            isFragment = true;
          }
        }
        // If existing text is a significant portion of this text (80%+), existing is a fragment
        if (textLength > existingLength && existingLength >= 20) {
          var overlapRatio = existingLength / textLength;
          if (overlapRatio >= 0.8 && textSignature.indexOf(existingSig.substring(0, Math.min(existingLength, 100))) !== -1) {
            isFragment = true;
          }
        }
      });
    ...
    // multi-line warn block produces huge DevTools stack traces that
  ...
  function detectMessageRole(element, index) {
    ...
    var hasAssistantIndicators = element.querySelector('[data-testid*="assistant"]') ||
                                  ...
                                  element.querySelector('img[alt*="Gemini"]') ||
  ...
  // Per-page throttle (same pattern as chatgpt-extractor.js / claude-extractor.js).
  // Prevents host-page lag when Gemini's router fires repeated pushState events.
  ...
  function extractConversation() {
    ...
    Logger.info('═══════════════════════════════════════════════════════════');
    Logger.info('Starting Gemini Conversation Extraction');
    Logger.info('═══════════════════════════════════════════════════════════');
    ...
    try {
      ...
      if (candidates.length === 0) {
        Logger.warn('No message candidates found - trying aggressive Gemini fallback...');
        
        // Aggressive Gemini fallback: Look for any div with substantial text
        ...
        allDivs.forEach(function(div) {
          ...
          var text = extractTextContent(div);
          if (!text || text.trim().length < 20) return;
          
          // Skip if this text is already captured by a parent
          var textKey = text.trim().substring(0, 50);
          if (seenText.has(textKey)) return;
          
          // Check if this div is nested inside another candidate
          var isNested = false;
          for (var i = 0; i < fallbackCandidates.length; i++) {
            if (fallbackCandidates[i].contains(div)) {
              isNested = true;
              break;
            }
          }
          
          if (!isNested && isElementVisible(div)) {
            fallbackCandidates.push(div);
            seenText.add(textKey);
          }
        });
        
        if (fallbackCandidates.length > 0) {
          Logger.log('Gemini fallback found ' + fallbackCandidates.length + ' text containers');
          candidates = fallbackCandidates;
        } else {
          Logger.warn('No message candidates found even with fallback');
          return { success: false, error: 'No message candidates found', messages: [] };
        }
      }
      ...
      if (realMessages.length === 0) {
        ...
        // Strategy A: Look for any article or div with substantial content
        var articles = root.querySelectorAll('article, [role="article"]');
        ...
        // Strategy B: Look for elements with message-like structure (Gemini-specific)
        var messageLike = root.querySelectorAll(
          '[class*="message"], [class*="Message"], [class*="turn"], [class*="Turn"], ' +
          '[class*="chat"], [class*="Chat"], [class*="block"], [class*="Block"], ' +
        ...
        // Strategy C: Look for any div with data attributes that might indicate messages
        var dataElements = root.querySelectorAll('[data-message-id], [data-message-author-role], [data-testid*="message"], [data-testid*="Message"]');
  ...
  function extractSidebar() {
    ...
    try {
      // Check if sidebar is collapsed/hidden and try to expand it
      var sidebarContainer = document.querySelector('.sidenav-with-history-container');
      if (sidebarContainer) {
        var isExpanded = sidebarContainer.classList.contains('expanded') || 
                        sidebarContainer.offsetWidth > 50 ||
                        sidebarContainer.style.display !== 'none';
        
        if (!isExpanded) {
          Logger.log('Sidebar appears collapsed, attempting to expand...');
          
          // Try to find and click the menu button to expand sidebar
          var menuButton = document.querySelector('[data-test-id="side-nav-menu-button"], [aria-label*="menu" i], [aria-label*="Menu" i]');
          if (menuButton) {
            Logger.log('Found menu button, clicking to expand sidebar...');
            menuButton.click();
            // Wait a bit for sidebar to expand
            var waitTime = 500;
            var waitStart = Date.now();
            while (Date.now() - waitStart < waitTime) {
              // Busy wait
            }
            Logger.log('Waited ' + waitTime + 'ms for sidebar to expand');
          }
        }
      }
      
      // Gemini sidebar strategies - try multiple approaches
      var chatLinks = [];
      ...
      if (nav) {
        ...
        // Gemini uses /app/[id] pattern - exclude account/sign-out links
        var allNavLinks = nav.querySelectorAll('a[href*="/app/"]');
        chatLinks = Array.from(allNavLinks).filter(function(link) {
          var href = link.getAttribute('href') || '';
          // Exclude account/sign-out links that have /app/ in query params
          return href.indexOf('accounts.google.com') === -1 && 
                 href.indexOf('SignOut') === -1 &&
                 href.match(/\/app\/[a-f0-9]+$/i) !== null; // Must end with /app/{id}
        });
        Logger.log('Strategy 1: Found nav with ' + chatLinks.length + ' chat links (filtered from ' + allNavLinks.length + ')');
        
        // Also look for any clickable elements in nav that might navigate to chats
        if (chatLinks.length === 0) {
          var navClickables = nav.querySelectorAll('[role="button"], button, [onclick], [data-href], [data-url]');
          Logger.log('Strategy 1b: Found ' + navClickables.length + ' clickable elements in nav');
          navClickables.forEach(function(elem) {
            var href = elem.getAttribute('href') || elem.getAttribute('data-href') || elem.getAttribute('data-url') || '';
            var onclick = elem.getAttribute('onclick') || '';
            var text = elem.innerText || elem.textContent || '';
            
            // Check if it references a chat ID
            var chatIdMatch = (href + ' ' + onclick).match(/\/app\/([a-f0-9]+)/i);
            if (chatIdMatch && href.indexOf('accounts.google.com') === -1) {
              var syntheticLink = {
                getAttribute: function(attr) {
                  if (attr === 'href') return '/app/' + chatIdMatch[1];
                  return null;
                },
                innerText: text,
                ...
                querySelectorAll: elem.querySelectorAll.bind(elem)
              };
              chatLinks.push(syntheticLink);
            }
          });
          if (chatLinks.length > 0) {
            Logger.log('Strategy 1b: Created ' + chatLinks.length + ' synthetic links from nav clickables');
          }
        }
      }
      
      // Strategy 2: Look for sidebar-specific classes/attributes
      if (chatLinks.length === 0) {
        var sidebarSelectors = [
          '.sidenav-with-history-container',  // Gemini's main sidebar container
          '[class*="sidebar"]',
          '[class*="Sidebar"]',
          '[class*="chat-list"]',
          '[class*="ChatList"]',
        ...
        for (var i = 0; i < sidebarSelectors.length; i++) {
          ...
          if (sidebar) {
            // First try regular links
            var allSidebarLinks = sidebar.querySelectorAll('a[href*="/app/"]');
            chatLinks = Array.from(allSidebarLinks).filter(function(link) {
              var href = link.getAttribute('href') || '';
              return href.indexOf('accounts.google.com') === -1 && 
                     href.indexOf('SignOut') === -1 &&
                     href.match(/\/app\/[a-f0-9]+$/i) !== null;
            });
            
            // If no links found, try clickable elements
            if (chatLinks.length === 0) {
              // Also look for library-item-card elements specifically
              var libraryItems = sidebar.querySelectorAll('library-item-card, [class*="library-item-card"], .library-item-card');
              if (libraryItems.length > 0) {
                Logger.log('Strategy 2c: Found ' + libraryItems.length + ' library-item-card elements in ' + sidebarSelectors[i]);
                libraryItems.forEach(function(item) {
                  var text = (item.innerText || item.textContent || '').trim();
                  // Skip UI elements
                  if (text.length < 5 || text.includes('New chat') || text.includes('My stuff') || text.includes('Gems')) {
                    return;
                  }
                  
                  // Try to match with stored conversations by title
                  Logger.log('Strategy 2c: Processing item with text: "' + text.substring(0, 50) + '"');
                  try {
                    var sessionData = VelocityGeminiExtractor.session.export();
                    if (sessionData && sessionData.conversations) {
                      var conversationsList = Array.isArray(sessionData.conversations) 
                        ? sessionData.conversations 
                        : Object.values(sessionData.conversations);
                      
                      Logger.log('Strategy 2c: Found ' + conversationsList.length + ' stored conversations to match against');
                      
                      // Normalize the item text (remove extra whitespace, get first line)
                      var itemTextNormalized = text.toLowerCase().trim();
                      itemTextNormalized = itemTextNormalized.split('\n')[0].split('\r')[0].trim();
                      
                      var matched = false;
                      for (var c = 0; c < conversationsList.length; c++) {
                        var conv = conversationsList[c];
                        if (conv && conv.title && conv.chatId) {
                          // Normalize stored title (get first line, remove "Upgrade to Google AI Plus" etc.)
                          var storedTitle = conv.title.toLowerCase().trim();
                          // Remove common suffixes
                          storedTitle = storedTitle.replace(/\nupgrade to google ai plus.*/i, '');
                          storedTitle = storedTitle.replace(/\nupgrade to google.*/i, '');
                          storedTitle = storedTitle.split('\n')[0].split('\r')[0].trim();
                          
                          Logger.log('Strategy 2c: Comparing "' + itemTextNormalized.substring(0, 30) + '" with "' + storedTitle.substring(0, 30) + '"');
                          
                          // Match if either contains the other (with at least 10 chars overlap)
                          var minLength = Math.min(itemTextNormalized.length, storedTitle.length);
                          if (minLength >= 10) {
                            // Check if one is a substring of the other
                            if (storedTitle.indexOf(itemTextNormalized) !== -1 || 
                                itemTextNormalized.indexOf(storedTitle) !== -1) {
                              var syntheticLink = {
                                getAttribute: function(attr) {
                                  if (attr === 'href') return '/app/' + conv.chatId;
                                  return null;
                                },
                                innerText: text,
                                parentElement: item.parentElement,
                                nextElementSibling: item.nextElementSibling,
                                previousElementSibling: item.previousElementSibling,
                                querySelectorAll: item.querySelectorAll.bind(item)
                              };
                              chatLinks.push(syntheticLink);
                              Logger.log('✅ Strategy 2c Matched: "' + text.substring(0, 40) + '" -> ' + conv.chatId + ' (stored: "' + conv.title.substring(0, 40) + '")');
                              matched = true;
                              break;
                            }
                          }
                      ...
                      if (!matched) {
                        Logger.log('Strategy 2c: No match found for "' + text.substring(0, 40) + '"');
                      }
                    } else {
                      Logger.log('Strategy 2c: No session data or conversations found');
                    }
                  } catch (e) {
                    Logger.log('Strategy 2c: Title matching error: ' + e.message);
                  }
                });
                
                if (chatLinks.length > 0) {
                  Logger.log('Strategy 2c: Created ' + chatLinks.length + ' links from library-item-card by title matching');
                }
              }
              
              var clickables = sidebar.querySelectorAll('[role="button"], button, [onclick], [data-href], [data-url], div[class*="chat"], div[class*="Chat"], div[tabindex], [tabindex], li, [class*="item"], [class*="Item"]');
              Logger.log('Strategy 2b: Found ' + clickables.length + ' clickable elements in ' + sidebarSelectors[i]);
              
              // Sample a few elements to understand their structure
              if (clickables.length > 0) {
                Logger.log('Inspecting clickable elements structure...');
                var sampleCount = Math.min(clickables.length, 10);
                for (var ci = 0; ci < sampleCount; ci++) {
                  var sample = clickables[ci];
                  var sampleHref = sample.getAttribute('href') || '';
                  var sampleOnclick = sample.getAttribute('onclick') || '';
                  var sampleData = Array.from(sample.attributes).slice(0, 5).map(function(attr) {
                    return attr.name + '="' + (attr.value || '').substring(0, 30) + '"';
                  }).join(', ');
                  var sampleText = (sample.innerText || sample.textContent || '').trim().substring(0, 60);
                  var sampleClass = (sample.className || '').substring(0, 80);
                  Logger.log('  Sample ' + (ci + 1) + ': text="' + sampleText + '", class="' + sampleClass + '", attrs=[' + sampleData + ']');
                }
              }
              
              // Also check child elements for links or chat IDs
              clickables.forEach(function(elem) {
                var href = elem.getAttribute('href') || elem.getAttribute('data-href') || elem.getAttribute('data-url') || '';
                var onclick = elem.getAttribute('onclick') || '';
                var text = elem.innerText || elem.textContent || '';
                
                // Check if this element or its children contain a link
                var childLink = elem.querySelector('a[href*="/app/"]');
                if (childLink) {
                  var childHref = childLink.getAttribute('href') || '';
                  if (childHref.indexOf('accounts.google.com') === -1 && childHref.match(/\/app\/[a-f0-9]+$/i)) {
                    chatLinks.push(childLink);
                    return;
                  }
                }
                
                // Check all data attributes for chat IDs
                var allDataAttrs = '';
                Array.from(elem.attributes).forEach(function(attr) {
                  if (attr.name.startsWith('data-') || attr.name === 'id' || attr.name === 'class') {
                    allDataAttrs += attr.value + ' ';
                  }
                });
                
                // Check if it references a chat ID in href, onclick, or data attributes
                var combined = href + ' ' + onclick + ' ' + allDataAttrs + ' ' + text;
                var chatIdMatch = combined.match(/\/app\/([a-f0-9]+)/i);
                
                // Also check for chat ID pattern directly (without /app/ prefix) - look for 16+ hex chars
                if (!chatIdMatch) {
                  // Look for patterns like "app/ID" or just long hex strings
                  var patterns = [
                    /app\/([a-f0-9]{16,})/i,
                    /gemini\.google\.com\/app\/([a-f0-9]{16,})/i,
                    /\b([a-f0-9]{16,})\b/i
                  ];
                  
                  for (var p = 0; p < patterns.length; p++) {
                    var match = combined.match(patterns[p]);
                    if (match) {
                      var potentialId = match[1];
                      // Verify it's a valid hex string and reasonable length
                      if (potentialId.length >= 16 && potentialId.length <= 32 && /^[a-f0-9]+$/i.test(potentialId)) {
                        chatIdMatch = ['/app/' + potentialId, potentialId];
                        break;
                      }
                    }
                  }
                ...
                // If we found a chat ID, create a synthetic link
                if (chatIdMatch && href.indexOf('accounts.google.com') === -1) {
                  var chatId = chatIdMatch[1];
                  // Skip if we already have this ID
                  var alreadyExists = false;
                  for (var cl = 0; cl < chatLinks.length; cl++) {
                    var existingHref = chatLinks[cl].getAttribute('href') || '';
                    if (existingHref.indexOf('/app/' + chatId) !== -1) {
                      alreadyExists = true;
                      break;
                    }
                  }
                  ...
                  if (!alreadyExists) {
                    var syntheticLink = {
                      getAttribute: function(attr) {
                        if (attr === 'href') return '/app/' + chatId;
                        return null;
                      },
                      innerText: text.trim() || 'Chat ' + chatId.substring(0, 8),
                      parentElement: elem.parentElement,
                      nextElementSibling: elem.nextElementSibling,
                      previousElementSibling: elem.previousElementSibling,
                      querySelectorAll: elem.querySelectorAll.bind(elem)
              ...
              if (chatLinks.length > 0) {
                Logger.log('Strategy 2b: Created ' + chatLinks.length + ' synthetic links from clickables');
              } else {
                Logger.log('Strategy 2b: No chat IDs found in clickable elements');
              }
            }
            
            if (chatLinks.length > 0) {
              sidebarContainer = sidebar;
              Logger.log('Strategy 2: Found sidebar via ' + sidebarSelectors[i] + ' with ' + chatLinks.length + ' chat links');
              break;
            }
          }
        }
      ...
      // Strategy 3: Look for any scrollable containers that might contain chat history
      if (chatLinks.length === 0) {
        var scrollContainers = document.querySelectorAll('[style*="overflow"], [class*="scroll"], [class*="Scroll"]');
        Logger.log('Strategy 3: Checking ' + scrollContainers.length + ' scroll containers');
        
        for (var sc = 0; sc < Math.min(scrollContainers.length, 10); sc++) {
          var container = scrollContainers[sc];
          var containerLinks = container.querySelectorAll('a[href*="/app/"]');
          var filteredLinks = Array.from(containerLinks).filter(function(link) {
            var href = link.getAttribute('href') || '';
            return href.indexOf('accounts.google.com') === -1 && 
                   href.indexOf('SignOut') === -1 &&
                   href.match(/\/app\/[a-f0-9]+$/i) !== null;
          ...
          if (filteredLinks.length > 0) {
            chatLinks = filteredLinks;
            sidebarContainer = container;
            Logger.log('Strategy 3: Found ' + chatLinks.length + ' chat links in scroll container');
            break;
          }
        }
      }
      
      // Strategy 4: Look for buttons or divs with onclick/data attributes that navigate to chats
      if (chatLinks.length === 0) {
        // Look for buttons/divs that might have chat IDs in data attributes or onclick handlers
        var interactiveElements = document.querySelectorAll('button[data-chat-id], div[data-chat-id], [onclick*="/app/"], [data-href*="/app/"]');
        Logger.log('Strategy 4: Found ' + interactiveElements.length + ' interactive elements with chat references');
        
        // Try to extract chat links from these elements
        interactiveElements.forEach(function(elem) {
          var chatId = elem.getAttribute('data-chat-id');
          var onclick = elem.getAttribute('onclick') || '';
          var dataHref = elem.getAttribute('data-href') || '';
          var combined = onclick + ' ' + dataHref;
          var chatIdMatch = combined.match(/\/app\/([a-f0-9]+)/i);
          
          if (chatId || chatIdMatch) {
            var id = chatId || chatIdMatch[1];
            // Create a synthetic link object
            var syntheticLink = {
              getAttribute: function(attr) {
                if (attr === 'href') return '/app/' + id;
                return null;
              },
              innerText: elem.innerText || elem.textContent || '',
              parentElement: elem.parentElement,
              nextElementSibling: elem.nextElementSibling,
              previousElementSibling: elem.previousElementSibling,
              querySelectorAll: elem.querySelectorAll.bind(elem)
        ...
        if (chatLinks.length > 0) {
          Logger.log('Strategy 4: Created ' + chatLinks.length + ' synthetic links from interactive elements');
        }
      }
      
      // Strategy 4: Look for any element with chat links (fallback) - but filter carefully
      if (chatLinks.length === 0) {
        var allDocLinks = document.querySelectorAll('a[href*="/app/"]');
        chatLinks = Array.from(allDocLinks).filter(function(link) {
          var href = link.getAttribute('href') || '';
          // Exclude account/sign-out links
          if (href.indexOf('accounts.google.com') !== -1 || href.indexOf('SignOut') !== -1) {
            return false;
          }
          // Must be a direct /app/{id} link, not in query params
          var match = href.match(/\/app\/([a-f0-9]+)/i);
          if (!match) return false;
          // Check if /app/ is in the path, not in query string
          var pathPart = href.split('?')[0].split('#')[0];
          return pathPart.indexOf('/app/') !== -1 && pathPart.match(/\/app\/[a-f0-9]+$/i) !== null;
        });
        if (chatLinks.length > 0) {
          Logger.log('Strategy 4: Found ' + chatLinks.length + ' chat links in document (filtered from ' + allDocLinks.length + ')');
        }
      }
      
      // Strategy 5: Look for any text content that might contain chat IDs (last resort)
      if (chatLinks.length === 0) {
        Logger.log('Strategy 5: Searching for chat IDs in DOM text content...');
        // Look for elements that might contain chat history text
        var allElements = document.querySelectorAll('div, span, li, button, a');
        var chatIdPattern = /[a-f0-9]{16,}/i; // Gemini chat IDs are typically 16+ hex chars
        var foundChatIds = new Set();
        
        for (var e = 0; e < Math.min(allElements.length, 500); e++) {
          var elem = allElements[e];
          var text = elem.innerText || elem.textContent || '';
          var href = elem.getAttribute('href') || '';
          var onclick = elem.getAttribute('onclick') || '';
          
          // Check if element or its attributes contain a chat ID pattern
          var combined = text + ' ' + href + ' ' + onclick;
          var matches = combined.match(/\/app\/([a-f0-9]{16,})/gi);
          
          if (matches) {
            matches.forEach(function(match) {
              var idMatch = match.match(/\/app\/([a-f0-9]+)/i);
              if (idMatch && idMatch[1].length >= 16) {
                var chatId = idMatch[1];
                if (!foundChatIds.has(chatId) && href.indexOf('accounts.google.com') === -1) {
                  ...
                  var syntheticLink = {
                    ...
                    innerText: text.substring(0, 100) || 'Chat ' + chatId.substring(0, 8),
                    parentElement: elem.parentElement,
                    nextElementSibling: elem.nextElementSibling,
                    previousElementSibling: elem.previousElementSibling,
                    querySelectorAll: elem.querySelectorAll.bind(elem)
        ...
        if (chatLinks.length > 0) {
          Logger.log('Strategy 5: Found ' + chatLinks.length + ' chat IDs in DOM content');
        }
      }
      
      // Strategy 6: Look for library-item-card and preview-item elements (Gemini-specific chat history items)
      // These are the actual chat history items in the sidebar
      if (chatLinks.length === 0) {
        // Look for library-item-card elements (the clickable chat items)
        var libraryItems = document.querySelectorAll('library-item-card, [class*="library-item-card"], .library-item-card, [role="button"][class*="library"]');
        Logger.log('Strategy 6a: Found ' + libraryItems.length + ' library-item-card elements');
        
        libraryItems.forEach(function(item) {
          var text = (item.innerText || item.textContent || '').trim();
          // Skip if it's not a chat item (too short, or contains UI text)
          if (text.length < 5 || text.includes('New chat') || text.includes('My stuff') || text.includes('Gems')) {
            return;
          }
          
          // Check for chat ID in various places
          var chatId = null;
          
          // Check jslog attribute (might contain chat ID)
          var jslog = item.getAttribute('jslog') || '';
          var jslogMatch = jslog.match(/app\/([a-f0-9]{16,})/i) || jslog.match(/\/([a-f0-9]{16,})/i);
          if (jslogMatch) {
            chatId = jslogMatch[1];
          }
          
          // Check all attributes for chat ID patterns
          if (!chatId) {
            Array.from(item.attributes).forEach(function(attr) {
              var value = attr.value || '';
              var match = value.match(/\/app\/([a-f0-9]{16,})/i) || value.match(/app\/([a-f0-9]{16,})/i);
              if (match) {
                chatId = match[1];
              }
            });
          ...
          // Check child elements (the actual clickable div)
          if (!chatId) {
            var clickableDiv = item.querySelector('[role="button"], [tabindex="0"]');
            if (clickableDiv) {
              Array.from(clickableDiv.attributes).forEach(function(attr) {
                var value = attr.value || '';
                var match = value.match(/\/app\/([a-f0-9]{16,})/i) || value.match(/app\/([a-f0-9]{16,})/i);
                if (match) {
                  chatId = match[1];
          ...
          // If we found a chat ID or have meaningful text, create a link
          if (chatId || text.length > 0) {
            if (!chatId) {
              // Try to match by title with stored conversations
              try {
                var sessionData = VelocityGeminiExtractor.session.export();
                if (sessionData && sessionData.conversations) {
                  var conversationsList = Array.isArray(sessionData.conversations) 
                  ...
                  // Normalize the item text (get first line, remove extra whitespace)
                  var itemTextNormalized = text.toLowerCase().trim();
                  itemTextNormalized = itemTextNormalized.split('\n')[0].split('\r')[0].trim();
                  
                  Logger.log('Strategy 6a: Matching "' + itemTextNormalized.substring(0, 40) + '" against ' + conversationsList.length + ' stored conversations');
                  ...
                  for (var c = 0; c < conversationsList.length; c++) {
                    ...
                    if (conv && conv.title && conv.chatId) {
                      ...
                      Logger.log('Strategy 6a: Comparing "' + itemTextNormalized.substring(0, 30) + '" with "' + storedTitle.substring(0, 30) + '"');
                      
                      // Match if either contains the other (with at least 10 chars)
                      var minLength = Math.min(itemTextNormalized.length, storedTitle.length);
                      if (minLength >= 10) {
                        if (storedTitle.indexOf(itemTextNormalized) !== -1 || 
                            itemTextNormalized.indexOf(storedTitle) !== -1) {
                          chatId = conv.chatId;
                          Logger.log('✅ Strategy 6a matched: "' + text.substring(0, 40) + '" -> ' + chatId + ' (stored: "' + conv.title.substring(0, 40) + '")');
                          break;
                        }
                      }
                    }
        ...
        if (chatLinks.length > 0) {
          Logger.log('Strategy 6a: Created ' + chatLinks.length + ' links from library-item-card elements');
        }
      }
      
      // Strategy 6b: Look for preview-item elements (fallback)
      if (chatLinks.length === 0) {
        var previewItems = document.querySelectorAll('[class*="preview-item"], .preview-item, [class*="preview"]');
        Logger.log('Strategy 6b: Found ' + previewItems.length + ' preview-item elements');
        
        if (previewItems.length > 0) {
          Logger.log('Sampling preview items...');
          for (var pi = 0; pi < Math.min(previewItems.length, 5); pi++) {
            var sample = previewItems[pi];
            var sampleText = (sample.innerText || sample.textContent || '').trim().substring(0, 60);
            var sampleAttrs = Array.from(sample.attributes).slice(0, 3).map(function(attr) {
              return attr.name + '="' + (attr.value || '').substring(0, 40) + '"';
            }).join(', ');
            Logger.log('  Preview item ' + (pi + 1) + ': text="' + sampleText + '", attrs=[' + sampleAttrs + ']');
          }
        }
        ...
        previewItems.forEach(function(item) {
          ...
          // Check all attributes for chat ID
          var allAttrs = '';
          Array.from(item.attributes).forEach(function(attr) {
            allAttrs += attr.value + ' ';
          });
          
          // Check for click handler or data attributes
          var clickHandler = item.getAttribute('jslog') || '';
          var dataAttrs = '';
          Array.from(item.attributes).forEach(function(attr) {
          ...
          var combined = href + ' ' + onclick + ' ' + allAttrs + ' ' + clickHandler + ' ' + dataAttrs;
          
          // Look for chat ID in various patterns
          var chatIdMatch = combined.match(/\/app\/([a-f0-9]{16,})/i);
          if (!chatIdMatch) {
            chatIdMatch = combined.match(/app\/([a-f0-9]{16,})/i);
          }
          if (!chatIdMatch) {
            // Look for long hex strings that might be chat IDs
            var hexMatch = combined.match(/\b([a-f0-9]{16,32})\b/i);
            if (hexMatch && /^[a-f0-9]+$/i.test(hexMatch[1])) {
              chatIdMatch = ['/app/' + hexMatch[1], hexMatch[1]];
            }
          }
          
          // If we found a chat ID or the text looks like a chat title, create a link
          if (chatIdMatch || (text.trim().length > 0 && text.length < 200)) {
            var chatId = chatIdMatch ? chatIdMatch[1] : null;
            
            // If no chat ID found but we have text, try to find it in parent or nearby elements
            if (!chatId) {
              var parent = item.parentElement;
              var depth = 0;
              while (parent && depth < 5) {
                var parentAttrs = '';
                Array.from(parent.attributes).forEach(function(attr) {
                  parentAttrs += attr.value + ' ';
                });
                var parentMatch = parentAttrs.match(/\/app\/([a-f0-9]{16,})/i) || parentAttrs.match(/app\/([a-f0-9]{16,})/i);
                if (parentMatch) {
                  chatId = parentMatch[1];
                  break;
                }
                parent = parent.parentElement;
                depth++;
            ...
            // Only create link if we have a valid chat ID (don't create placeholder links)
            // If no chat ID, try to match with stored conversations first
                    ...
                    if (conv && conv.title && conv.chatId) {
                      ...
                      if (minLength >= 10) {
                        if (storedTitle.indexOf(itemTextNormalized) !== -1 || 
                            itemTextNormalized.indexOf(storedTitle) !== -1) {
                          chatId = conv.chatId;
                          Logger.log('Strategy 6b matched: "' + text.substring(0, 40) + '" -> ' + chatId);
                          break;
                        }
                      }
                    }
            ...
            // Only create link if we have a valid chat ID
            if (chatId) {
              var syntheticLink = {
                getAttribute: function(attr) {
                  if (attr === 'href') return '/app/' + chatId;
        ...
        if (chatLinks.length > 0) {
          Logger.log('Strategy 6: Created ' + chatLinks.length + ' links from preview-item elements');
        }
      }
      
      // Fallback: Use stored conversations from session if DOM extraction fails
      if (chatLinks.length === 0) {
        Logger.log('No chat links found in DOM, trying fallback: using stored conversations...');
        try {
          var sessionData = VelocityGeminiExtractor.session.export();
          if (sessionData && sessionData.conversations) {
            ...
            if (Array.isArray(sessionData.conversations)) {
              conversationsList = sessionData.conversations;
            } else if (typeof sessionData.conversations === 'object') {
              // Convert object to array
              Object.keys(sessionData.conversations).forEach(function(chatId) {
                var conv = sessionData.conversations[chatId];
                if (conv && conv.chatId) {
            ...
            if (conversationsList.length > 0) {
              Logger.log('Found ' + conversationsList.length + ' stored conversations, creating sidebar entries...');
              // Sort by lastUpdated (most recent first)
              conversationsList.sort(function(a, b) {
                return (b.lastUpdated || 0) - (a.lastUpdated || 0);
              });
              
              conversationsList.forEach(function(conv) {
                if (conv && conv.chatId) {
                  var syntheticLink = {
                    getAttribute: function(attr) {
                      if (attr === 'href') return '/app/' + conv.chatId;
                    ...
                    innerText: conv.title ? conv.title.split('\n')[0].trim() : 'Chat ' + conv.chatId.substring(0, 8),
                    parentElement: null,
              ...
              if (chatLinks.length > 0) {
                Logger.log('Fallback: Created ' + chatLinks.length + ' sidebar entries from stored conversations');
              }
            } else {
              Logger.log('Fallback: No conversations found in session data');
            }
          } else {
            Logger.log('Fallback: No session data or conversations found');
          }
        } catch (e) {
          Logger.error('Fallback failed: ' + e.message);
          Logger.error('Fallback error stack: ' + (e.stack || 'no stack'));
        }
      }
      
      if (chatLinks.length === 0) {
        Logger.warn('No chat links found in sidebar after all strategies');
        Logger.log('Debug: Checking for hidden/collapsed sidebar...');
        // Check if there are any elements that might be a hidden sidebar
        var hiddenElements = document.querySelectorAll('[style*="display: none"], [hidden], [aria-hidden="true"]');
        Logger.log('Found ' + hiddenElements.length + ' potentially hidden elements');
        
        // Check for drawer/menu elements
        var drawerElements = document.querySelectorAll('[class*="drawer"], [class*="Drawer"], [class*="menu"], [class*="Menu"]');
        Logger.log('Found ' + drawerElements.length + ' drawer/menu elements');
        
        return { success: false, error: 'No chats found', chats: [] };
      }
      
      Logger.log('✅ Found ' + chatLinks.length + ' chat links to process');
      ...
      chatLinks.forEach(function(link, index) {
        ...
        Logger.log('Processing link ' + (index + 1) + ': href=' + href + ', title="' + title + '"');
        
        // Extract chat ID from href (Gemini uses /app/[id] pattern)
        // e.g., /app/cf8e9107d89c3e59 or /app/e11e798e99a6dc31
        // Must be in the path, not in query params
        var pathPart = href.split('?')[0].split('#')[0];
        var chatIdMatch = pathPart.match(/\/app\/([a-f0-9]+)$/i);
        if (!chatIdMatch && pathPart.indexOf('gemini.google.com') !== -1) {
          chatIdMatch = pathPart.match(/gemini\.google\.com\/app\/([a-f0-9]+)$/i);
        }
        var chatId = chatIdMatch ? chatIdMatch[1] : null;
        
        Logger.log('  Chat ID extracted: ' + (chatId || 'null'));
        
        // Skip if no valid chat ID or duplicate
        if (!chatId) {
          Logger.log('  ⏭️ Skipping: No valid chat ID (href: ' + href + ')');
          return;
        }
        if (seenIds.has(chatId)) {
          Logger.log('  ⏭️ Skipping: Duplicate chat ID');
          return;
        }
        seenIds.add(chatId);
        ...
        if (!title || title.length < 1) {
          ...
          if (parent) {
            ...
            if (parentText && parentText.length > title.length) {
              title = parentText;
              Logger.log('  Found title from parent: "' + title.substring(0, 50) + '"');
            }
          }
          
          // If still no title, try to find text in nearby elements
          if (!title || title.length < 1) {
            var nextSibling = link.nextElementSibling;
            if (nextSibling) {
              var siblingText = nextSibling.innerText ? nextSibling.innerText.trim() : '';
              if (siblingText && siblingText.length > 0) {
                title = siblingText;
                Logger.log('  Found title from sibling: "' + title.substring(0, 50) + '"');
              }
            }
            
            // Try previous sibling as well
            if (!title || title.length < 1) {
              var prevSibling = link.previousElementSibling;
              if (prevSibling) {
                var prevText = prevSibling.innerText ? prevSibling.innerText.trim() : '';
                if (prevText && prevText.length > 0) {
                  title = prevText;
                  Logger.log('  Found title from previous sibling: "' + title.substring(0, 50) + '"');
                }
              }
            }
            ...
            if (!title || title.length < 1) {
              var childTextElements = link.querySelectorAll('span, div, p');
              for (var ct = 0; ct < Math.min(childTextElements.length, 5); ct++) {
                var childText = childTextElements[ct].innerText ? childTextElements[ct].innerText.trim() : '';
                if (childText && childText.length > 0) {
                  title = childText;
                  Logger.log('  Found title from child: "' + title.substring(0, 50) + '"');
                  break;
                }
              }
            }
        ...
        // Skip if still no title (but allow if we have a valid chat ID)
        if (!title || title.length < 1) {
          // For Gemini, if we have a valid chat ID but no title, use a default title
          title = 'Chat ' + chatId.substring(0, 8);
          Logger.log('  Using default title: "' + title + '"');
        }
        
        // Build full URL
        var fullUrl = href.startsWith('http') ? href : 'https://gemini.google.com' + href;
        
        // Check if this is the current active chat
        var currentPath = window.location.pathname;
        var isActive = currentPath === href || currentPath.indexOf('/app/' + chatId) !== -1;
  ...
  function extractAll() {
    Logger.info('═══════════════════════════════════════════════════════════');
    Logger.info('Extracting Full Gemini Data (Conversation + Sidebar)');
    Logger.info('═══════════════════════════════════════════════════════════');
    ...
    Logger.success('Full extraction completed:');
    Logger.log('  • Conversation: ' + payload.conversation.messageCount + ' messages');
    Logger.log('  • Sidebar: ' + payload.sidebar.totalChats + ' chats');
    Logger.log('  • Time: ' + payload.extractionTime);
    Logger.info('═══════════════════════════════════════════════════════════');
  ...
  if (typeof window.SmartTriggerManager !== 'undefined') {
    ...
    Logger.info('✅ Smart Trigger Manager initialized with settings: minWordCountDelta=10, minTimeBetweenUpdates=1000, maxUpdatesPerHour=60, streamingDebounceTime=1000, minMessages=5');
  } else {
  ...
  var SmartTracker = {
    ...
    start: function(callback) {
      ...
      // Only run when actually on a conversation page — avoids hammering the
      // host main thread on home/new-chat URLs.
      if (DataCache.isConversationPage()) {
    ...
    handleUrlChange: function(callback) {
      ...
      if (newChatId !== oldChatId && newChatId) {
        ...
        // biggest source of host-page lag. The extractConversation() throttle
        // further dedupes within MIN_INTERVAL_MS.
        ...
        // Same chat - just update if needed (only when there IS a chat; home
        // pushState should not trigger extraction).
    ...
    performSmartExtraction: function(callback, trigger) {
      ...
      // Cheap early-out: skip heavy DOM scans / payload assembly when there's
      // no active conversation. Prevents extraction storms on home/new-chat
      // URLs where every internal pushState would otherwise re-run the pipeline.
      ...
      if (chatId && conversationData.success) {
        ...
        if (conversationData.messages && conversationData.messages.length > 0) {
          Logger.success('✅ Saved ' + conversationData.messages.length + ' messages for chat: ' + chatId.substring(0, 8));
  ...
  window.VelocityGeminiExtractor = {
     ...
     * Can be called from browser console: VelocityGeminiExtractor.triggerContextEngine()
     */
    triggerContextEngine: function() {
      log('[Gemini Extractor] 🚀 Manual ContextEngine API trigger initiated');
      ...
      try {
        ...
        if (!conversationData.success || !conversationData.messages || conversationData.messages.length === 0) {
          error('[Gemini Extractor] ❌ No conversation data to process');
          return { success: false, error: 'No conversation data available' };
        }

        log('[Gemini Extractor] 📊 Extracted data for ContextEngine:', {
        ...
        function handleBridgeResponse(event) {
          if (event.data.type === 'CONTEXT_ENGINE_BRIDGE_RESPONSE' &&
            ...
            if (event.data.error) {
              error('[Gemini Extractor] ❌ Bridge error:', event.data.error);
            } else if (event.data.response && event.data.response.success) {
              log('[Gemini Extractor] ✅ ContextEngine API triggered successfully');
              log('[Gemini Extractor] 📄 API Response:', event.data.response.contextResult);
            } else {
              error('[Gemini Extractor] ❌ ContextEngine API failed:', event.data.response?.error);
            }
          }
        }
        ...
        setTimeout(function() {
          window.removeEventListener('message', handleBridgeResponse);
          warn('[Gemini Extractor] ⏰ Bridge response timeout for manual trigger');
        }, 10000);

        return { success: true, message: 'ContextEngine API trigger initiated' };

      } catch (error) {
              error('[Gemini Extractor] 💥 Error triggering ContextEngine API:', error);
    ...
    showJSON: function(data, label) {
      if (!data) {
        warn('[Gemini Extractor] No data provided');
        return;
      }
      // Validate data structure before showing
      if (data.messages && !Array.isArray(data.messages)) {
        warn('[Gemini Extractor] ⚠️ Warning: messages is not an array');
        data.messages = [];
      }
    ...
    validateData: function(data) {
      ...
      if (data.success === undefined) {
        issues.push('Missing success flag');
      }
      ...
      if (data.messageCount !== undefined && typeof data.messageCount !== 'number') {
        issues.push('messageCount is not a number');
      }
      
      if (data.messages && data.messageCount !== undefined) {
        if (data.messages.length !== data.messageCount) {
          issues.push('messageCount (' + data.messageCount + ') does not match messages.length (' + data.messages.length + ')');
        }
      }
      ...
      if (data.messages) {
        data.messages.forEach(function(msg, idx) {
          ...
          if (msg.content === undefined) issues.push('Message ' + idx + ' missing content');
          if (typeof msg.timestamp !== 'number') issues.push('Message ' + idx + ' has invalid timestamp');
        });
      }
  ...
  Logger.success('Gemini Extractor initialized');
        ...
        function handleBridgeResponse(event) {
          if (event.data.type === 'CONTEXT_ENGINE_BRIDGE_RESPONSE' &&
            ...
            if (event.data.error) {
              ...
              warn('[Auto-Trigger] ⚠️ ContextEngine API auto-trigger failed:', event.data.response?.error);
            }
          }
        }
  ...
  function autoStartTracking() {
    if (!CONFIG.AUTO_TRACK) {
      Logger.info('Auto-tracking disabled. Use VelocityGeminiExtractor.startTracking() manually.');
      return;
    }
    ...
    Logger.info('');
    Logger.info('📦 View collected: VelocityGeminiExtractor.session.summary()');
    Logger.info('💾 Export all: VelocityGeminiExtractor.session.export()');
    Logger.info('═══════════════════════════════════════════════════════════');
  }
  ...
  if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
      if (request.type === 'CLEANUP_EXTENSION_DATA') {
        log('[Gemini Extractor] 🧹 Received cleanup message, resetting session data');
        ...
        try {
          ...
          var STORAGE_KEY = 'VelocityGeminiExtractor_session';
          localStorage.removeItem(STORAGE_KEY);

          log('[Gemini Extractor] ✅ Session data cleared');
        } catch (e) {
          error('[Gemini Extractor] ❌ Error clearing session data:', e);
        }
