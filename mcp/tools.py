    ...
    return [
        Tool(
            ...
            description=(
                ...
                "Use inject_context when the surrounding agent session needs a readable summary of stored memory."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "target_ai": {
                        ...
                        "description": "Target AI surface: claude, chatgpt, gpt-5, gemini, groq, cursor, bolt, replit, gamma, midjourney",
                    },
                    "prompt_mode": {
                        "type": "string",
                        "enum": list(PROMPT_MODE_VALUES),
                        "description": "Prompt system mode: normal or caveman",
                    },
                },
                "required": ["prompt"],
            },
        ...
        Tool(
            ...
            description=(
                ...
                "For best results, include previous_enhanced_prompt and previous annotation metadata."
            ),
            inputSchema={
                "type": "object",
                "properties": {
        ...
        Tool(
            ...
            description=(
                ...
                "Use this when you need a human-readable summary of expertise, stack, current work, and preferences."
            ),
            inputSchema={"type": "object", "properties": {}},
        ),
    ]
...
async def _handle_enhance(args: dict) -> list[TextContent]:
    ...
    try:
        target_ai = normalize_target_ai(args.get("target_ai"))
        prompt_mode = normalize_prompt_mode(args.get("prompt_mode"))
    except ValueError as exc:
        return [TextContent(type="text", text=f"Velocity: {exc}")]
    user_id = _user_id()
    bundle = prompt_bundle("enhance", prompt_mode)
    ...
    try:
        ctx_payload = json.loads(ctx_block) if ctx_block else None
    except json.JSONDecodeError:
        ctx_payload = ctx_block or None
    ...
    store.update_after_enhancement(
        ...
        result.get("prompt_mode", prompt_mode),
    )
...
async def _handle_refine(args: dict) -> list[TextContent]:
    ...
    try:
        ...
        return [TextContent(type="text", text=f"Velocity Refine: {exc}")]
    bundle = prompt_bundle("refine", prompt_mode)
        ...
        json.dumps(
            {
                ...
                "target_ai": target_ai,
                "prompt_mode": prompt_mode,
                "previous_enhanced_prompt": previous_enhanced_prompt,
                "previous_framework_used": previous_framework_used,
                "previous_pe_techniques_applied": previous_pe_techniques_applied,
                "previous_placeholder_fields": previous_placeholder_fields,
                "previous_annotated_segments": previous_annotated_segments,
    ...
    raw = await _refine_complete(bundle.text, user_message)
...
async def _refine_complete(system_prompt: str, user_message: str) -> str:
    return await complete(system_prompt, user_message, temperature=0.3)
